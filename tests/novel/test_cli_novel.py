...
class TestNovelImport:
    """Test the 'novel import' command."""
    ...
    @patch("src.novel.services.import_service.ImportService.extract_characters")
    @patch("src.novel.services.import_service.ImportService.extract_world_setting")
    @patch("src.novel.config.load_novel_config")
    @patch("src.llm.llm_client.create_llm_client")
    def test_import_success(
        self, mock_create_llm, mock_load_config, mock_world, mock_chars,
        sample_file, tmp_path
    ):
        mock_create_llm.return_value = MagicMock()
        mock_load_config.return_value = MagicMock(model_dump=MagicMock(return_value={}))
        mock_chars.return_value = [{"name": "张三", "role": "主角"}]
        mock_world.return_value = {"era": "现代", "location": "北京", "rules": [], "terms": {}}
    ...
    def test_import_missing_file(self, tmp_path):
        ...
        # Click validates --file with exists=True
        assert result.exit_code != 0
    ...
    def test_import_no_auto_split(
        ...
        mock_world.return_value = {"era": "架空", "location": "未知", "rules": [], "terms": {}}
