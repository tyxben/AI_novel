...
- 无意义输入（如无到期伏笔、空文本）返回 ``score=None`` + ``details["status"]``,
  下游报告层把 None 视为"本维度无数据"（display "-"），不参与 regression 对比。
  见 Phase 5 review fix H1/H2。
"""
...
def evaluate_foreshadow_payoff(
    ...
    "命中即兑现"是保守近似——不做语义判断，只看是否显式提及。

    **无到期伏笔** / 空 snapshot / 缺失 ledger 时返回 ``score=None`` +
    ``details["status"] = "no_due_foreshadowings"``（0/0 不是"满分"——下游应
    显示 "-" / "N/A" 并跳过 regression 对比）。修复 Phase 5 review H1。
    ...
    Returns:
        ...
        ``total==0`` 时额外含 ``status="no_due_foreshadowings"``。
    """
    try:
    ...
    if total == 0:
        # H1 fix: 0/0 不是"满分"——返回 score=None 让下游显示"-"
...
def evaluate_ai_flavor(
    ...
    if not text or not text.strip():
        # H2 fix: 空文本 score=0 是语义错误（0 表示"零 AI 味"）。改 score=None +
        # status="empty_text"，下游按"无数据"处理。
        return DimensionScore(
            key="ai_flavor_index",
            score=None,
            scale="0-100",
            ...
            details={
                ...
                "genre": genre,
                "status": "empty_text",
                "warnings": ["empty_text: 无法评估 AI 味，文本为空"],
            },
        )
