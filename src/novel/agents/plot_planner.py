...
11. 如果提供了"连续性约束"，第一个场景必须承接上章遗留事项，其title和summary必须体现"承接上章"而非"新事件开场"。场景规划中不能出现连续性约束中"禁止违反"的项目
12. 第一个场景的summary必须包含：上章结尾角色所在位置、正在进行的动作、或需要回应的悬念。绝对禁止第一场景跳过上章结尾的时间/空间/事件
  ...
  "scenes": [
    {
      ...
      "transition_from": "上章遗留事项的简述，仅第一个场景需要" 或 null,
